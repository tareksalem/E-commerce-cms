...
// function for making a promises
    ...
    document.liveEvent = function (event, target, cb, option) {
        document.addEventListener(event, function (e) {
            ...
            if (typeof target == "string") {
                ...
                var targetElement = Array.from(targetElements).find((el) => {return el == e.target});
                if (targetElement) {
                    return cb(e);
                } else {
                    return null;
            ...
            if (self.length !== undefined && self.length > 0) {
                try {
                    Array.from(self).forEach((elem) => {
                        if(typeof elem == "object" && "nodeType" in elem &&
                        elem.nodeType === 1 && elem.cloneNode){
                            try{
    ...
    this.require = function (path, parent) {
        ...
        if (typeof parent === "string") {
            ...
            let getScript = Array.from(allScripts).find((scr) => {return scr.src === path});
            if (getScript) {
                return null;
            } else {
                    ...
                    controllerElements.forEach(function (controller) {
                        ...
                        function responseThisOne(targetElement) {
                            ...
                            if (thisResponseType === "animating") {
                                ...
                                var thisAnimationOptions =
                                    targetElement.getAttribute("thisAnimationOptions");
                                var ObjOptions = {};
                        ...
                        function responseThisTwo(targetElement) {
                            ...
                            if (thisResponseType2 === "animating") {
                                ...
                                var thisAnimationOptions2 =
                                    targetElement.getAttribute("thisAnimationOptions2");
                                var ObjOptions = {};
